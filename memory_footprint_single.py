...
for Q in [3, 6, 9, 12, 15]:
    ...
    method = SISONetRegressor(
        n_features=n_features_per_source,
        n_hidden=32,
        n_shared=1,
        n_specific=2,
    ...
    with mem_tracker as mt:
        for i in range(2):
            input_batch = torch.rand((N, n_features_per_source), device=device, dtype=dtype)
